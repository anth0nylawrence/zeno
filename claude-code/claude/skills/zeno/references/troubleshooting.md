...
- Ensure hooks are configured in `.claude/settings.json` or `~/.claude/settings.json`.
- Run `/hooks` and confirm the Zeno hooks are listed.
- Check that hook scripts are executable and reachable.
- Check `.claude/zeno/notify.log` to confirm whether the Stop hook is firing.
...
- Inspect the transcript referenced by `payload["transcript_path"]` if available.
...
Technical:
- Ensure `.claude/zeno/state/<thread-id>.json` exists.
- Run `scripts/zeno_context_bridge.py --zeno-root /path/to/repo/.claude/zeno`.
...
Technical:
- Verify `CLAUDE_CODE_ENABLE_TELEMETRY=1` and OTEL env vars are set.
- Ensure the collector is running with `configs/otel-collector.yaml` (OTLP gRPC default).
